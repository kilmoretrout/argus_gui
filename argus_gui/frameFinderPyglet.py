...
class frameFinder():
    def __init__(self, ifile, end, width = 640, height = 480, factor = 4, offset = 0, rgb = True):
        self.file = ifile
        ...
        self.x = 0
        self.y = 0
        self.w = int(self.movie.get(cv2.cv.CV_CAP_PROP_FRAME_WIDTH))
        self.h = int(self.movie.get(cv2.cv.CV_CAP_PROP_FRAME_HEIGHT))
        self.ow = int(self.movie.get(cv2.cv.CV_CAP_PROP_FRAME_WIDTH))
        self.oh = int(self.movie.get(cv2.cv.CV_CAP_PROP_FRAME_HEIGHT))
        self.frameCount = int(self.movie.get(cv2.cv.CV_CAP_PROP_FRAME_COUNT))
        self.frame_rate = self.movie.get(cv2.cv.CV_CAP_PROP_FPS)
    ...
    def getFrame(self, n, mode = 'pygame'):
        if n - 1 + self.offset >= 0 and n - 1 + self.offset <= self.frameCount - 1:
            self.movie.set(cv2.cv.CV_CAP_PROP_POS_MSEC, (n - 1 + self.offset)*self.frame_msec)
    ...
    def format(self, image, size = None):
        # flip
        ...
        if self.image is not None:
            if self.image[0] == n:
                ...
                if (y-wy >= 0) and (y+wy < im.shape[0]) and (x-wx >= 0) and (x+wx < im.shape[1]):
                    return self.format(im[y-wy: y+wy, x-wx: x+wx], size = (wf*wy,wf*wx))
                else:
                    return None
        if n - 1 + self.offset >= 0 and n - 1 + self.offset <= self.frameCount - 1:
            self.movie.set(cv2.cv.CV_CAP_PROP_POS_MSEC, (n - 1 + self.offset)*self.frame_msec)
