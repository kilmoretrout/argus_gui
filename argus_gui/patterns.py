...
class PatternFinder:
    def __init__(self, rows, cols, spacing, ofile, ifile, start = None, stop = None, display = True, dots = True):
        ...
        if not self.start is None:
            self.start = int(np.floor(self.start*self.movie.get(cv2.cv.CV_CAP_PROP_FPS)))
        else:
            self.start = 0
        if not self.stop is None:
            self.stop = int(np.floor(self.stop*self.movie.get(cv2.cv.CV_CAP_PROP_FPS)))
        else:
            self.stop = None
    
        if ((self.stop is None) or \
                (int(self.stop) < 0) or \
                (int(self.stop) > int(self.movie.get(cv2.cv.CV_CAP_PROP_FRAME_COUNT)))):
            self.stop = int(self.movie.get(cv2.cv.CV_CAP_PROP_FRAME_COUNT))
        if ((self.start < 0) or \
        ...
        sys.stdout.flush()
        self.imageSize  = (int(self.movie.get(cv2.cv.CV_CAP_PROP_FRAME_WIDTH)),
