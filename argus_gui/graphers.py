...
class wandGrapher():
    def __init__(self, key, nppts, nuppts, scale, ref, indices, ncams, npframes, nupframes = None, name = None, temp = None, display = True, uvs = None, nRef = 0, order = None, report = True):
        self.nppts = nppts
        self.nuppts = nuppts
        self.scale = scale
        self.ref = ref
    ...
    def getCoefficients(self, xyz, uv):
        ...
        for k in range(len(errors)):
            if errors[k] >= 3*stderr + merr:
                ptsi.append(k)
                if self.nRef - 1 < k < pb_1:
                    #if not self.indices['paired'][0][k] in frames:
                    outliers.append([self.indices['paired'][0][k] + 1, uv[k], 'Paired (set 1)', errors[k]])
                    #frames.append(self.indices['paired'][0][k])
                elif pb_1 <= k < self.nppts:
                    #if not self.indices['paired'][1][k - len(self.indices['paired'][0])] in frames:
                    outliers.append([self.indices['paired'][1][k - len(self.indices['paired'][0])] + 1, uv[k], 'Paired (set 2)', errors[k]])
                    #frames.append(self.indices['paired'][1][k - len(self.indices['paired'][0])])
                elif self.nppts <= k < self.nuppts + self.nppts:
                    try:
                        #if not upindices[(k - self.nppts)] in frames:
                        outliers.append([upindices[(k - self.nppts)] + 1, uv[k], 'Unpaired', errors[k]])
    ...
    def graph(self):
        ...
        xyzs_trimmed = xyzs[self.nRef:,:]
        
        # If we've got paired points, define a scale
        if self.nppts != 0:
            paired = xyzs_trimmed[:self.nppts]
        ...
        ptsi = []
        for uv in self.uvs:
            cos, error, outlier, ind = self.getCoefficients(xyzs*factor, uv)
