...
class sbaArgusDriver():
    ...
    def count(self, pts):
        ...
        for k in range(pts.shape[0]):
            for j in range(pts.shape[1]/(2*self.ncams)):
                ...
                for i in range(len(_)/2):
                    if not _[i*2]:
                        counts[i] += list(_).count(False)/2 - 1
        return counts
    ...
    def rearrange(self): 
        ...
        if best != self.ncams - 1:
            ...
            if self.uppts is not None:
                for k in range(self.uppts.shape[1]/(2*self.ncams)):
    ...
    def fix(self):
        ...
        grapher = wandGrapher(key, self.nppts, self.nuppts, self.scale, refBool, self.indices, self.ncams, npframes, nupframes, self.name, self.temp, self.display, uvs, nRef, self.order, self.report)
        if self.display:
        ...
        if self.report:
            if len(outliers) == 0:
                ...
                if self.display:
                    ...
                    print table.draw()
                    go_again = raw_input('Try again without these outliers? (Y/n): ')
